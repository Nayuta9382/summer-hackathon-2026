...
export default function SensorList({ sensors, tagMap, onToggle, onOpen }: Props) {
    ...
    const filtered = useMemo(() => {
        let list = sensors;
        if (activeTag) list = list.filter((s) => s.tagIds.includes(activeTag));
        if (query.trim()) {
            const q = query.trim().toLowerCase();
            list = list.filter((s) => s.name.toLowerCase().includes(q) || s.ip.includes(q));
        }
        const arr = [...list];
        switch (sort) {
            case 'name':
                ...
                break;
            case 'ip':
                arr.sort((a, b) => a.ip.localeCompare(b.ip));
                break;
            case 'recent':
            default:
                arr.sort((a, b) => b.lastDetectedAt.localeCompare(a.lastDetectedAt));
        }
        return arr;
        ...
        <section className="rounded-2xl border border-background-200 bg-background-50 overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center gap-3 px-4 md:px-5 py-4 border-b border-background-200">
                ...
                <div className="flex flex-col sm:flex-row gap-2 md:ml-auto md:items-center">
                    <div className="relative sm:w-60">
                        ...
                        <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="センサー名・IPで検索" className="pl-9 text-sm" />
                    </div>
                    <div className="sm:w-40">
                        <Select value={sort} onChange={(e) => setSort(e.target.value as SortKey)}>
                            <option value="recent">直近の検知順</option>
                            <option value="name">名前順</option>
                            <option value="ip">IP順</option>
                        </Select>
                    </div>
                </div>
            </div>
            ...
            <div className="p-3 md:p-4 space-y-2.5">
                {filtered.length === 0 ? (
                    ...
                    filtered.map((s) => <SensorCard key={s.id} sensor={s} tagMap={tagMap} onToggle={onToggle} onOpen={onOpen} />)
                )}
            </div>
        </section>
    );
