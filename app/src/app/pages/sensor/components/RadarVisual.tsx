...
const hexColor: Record<Sensor['status'], { color: string; bg: string; icon: string; sweepSpeed: string }> = {
    normal: { color: '#10B981', bg: '#E7F9F1', icon: 'ri-radar-line', sweepSpeed: '3.2s' },
    ...
    disabled: { color: '#94A3B8', bg: '#F1F5F9', icon: 'ri-pause-circle-line', sweepSpeed: '0s' },
    offline: { color: '#FB7185', bg: '#FFEEF1', icon: 'ri-wifi-off-line', sweepSpeed: '0s' },
};

export default function RadarVisual({ sensor }: Props) {
    const c = hexColor[sensor.status];
    const detecting = sensor.status === 'detecting';
    const isIdle = sensor.status === 'normal' || sensor.status === 'unconfirmed';
    const swept = sensor.status === 'normal' || detecting || sensor.status === 'unconfirmed';
    ...
    return (
        <div className="relative flex items-center justify-center w-[220px] h-[220px] md:w-[280px] md:h-[280px] shrink-0">
            ...
            <div
                ...
                style={{ backgroundColor: c.bg, borderColor: `${c.color}55`, opacity: sensor.status === 'disabled' ? 0.85 : 1 }}
            >
                {/* レーダー走査 */}
                {swept && (
                ...
                <div
                    className={`relative flex items-center justify-center w-20 h-20 md:w-24 md:h-24 rounded-full text-white ${
                        detecting ? 'animate-soft-bounce' : sensor.status === 'unconfirmed' ? 'animate-pulse-soft' : ''
                    }`}
            ...
            {sensor.status === 'offline' && (
                <span className="absolute w-[150px] h-[150px] md:w-[180px] md:h-[180px] flex items-center justify-center pointer-events-none" style={{ transform: 'rotate(-12deg)' }}>
                    <span className="block w-full border-t-2 border-dashed" style={{ borderColor: c.color }} />
                </span>
            )}
        </div>
    );
