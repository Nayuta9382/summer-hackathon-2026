...
function NotificationsInner() {
    ...
    const allItems = useMemo(() => buildNotifications(), []);
    const tagMap = useMemo<Record<string, Tag>>(() => buildTagMap(), []);
    ...
    const filtered = useMemo(() => {
        ...
        if (query.trim()) {
            ...
            list = list.filter((n) => n.sensorName.toLowerCase().includes(q) || n.sensorIp.includes(q));
        }
        return list;
                ...
                <main className="flex-1 w-full max-w-[1200px] mx-auto px-4 md:px-6 py-5 md:py-8 space-y-5 md:space-y-6">
                    ...
                    <section className="rounded-2xl border border-background-200 bg-background-50 overflow-hidden">
                        ...
                        <div className="p-4 md:p-6">
                            <NotificationTimeline items={filtered} tagMap={tagMap} onOpen={(id) => router.push(`/sensor/${id}`)} />
                        </div>
                    </section>
                </main>
            </div>
