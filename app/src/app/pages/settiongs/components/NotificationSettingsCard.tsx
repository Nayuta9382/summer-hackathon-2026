...
const destTabs: { key: DestType; label: string; icon: string; hint: string }[] = [
    { key: 'slack', label: 'Slack', icon: 'ri-slack-line', hint: 'メールアドレスで通知します' },
...
export default function NotificationSettingsCard({ onSave }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!destValue.trim()) {
            // 未入力は保存しない（デモではトーストのみ）
            onSave({ sound, soundType, dest, destValue: destValue.trim() });
            return;
        }
        onSave({ sound, soundType, dest, destValue: destValue.trim() });
    };
            ...
            <form onSubmit={handleSubmit} className="p-5 md:p-6 space-y-5">
                ...
                <div className="rounded-xl border border-background-200 bg-background-50 p-4">
                    ...
                    <div className="mt-4 max-w-sm">
                        <Field label={dest === 'slack' ? 'メールアドレス' : 'LINE ID'} htmlFor="dest-value">
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    placeholder={dest === 'slack' ? 'you@example.com' : 'LINEのIDを入力'}
                                    className="pl-9"
                                    type={dest === 'slack' ? 'email' : 'text'}
                                />
                            </div>
                        </Field>
                    </div>
