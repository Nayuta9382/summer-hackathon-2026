...
interface Props {
    ...
    tags: TagResponse[];
    onCreated?: () => void;
    onUpdate: (tagId: number, data: { name: string; color: string }) => void;
...
export default function TagManagerModal({ open, onClose, tags, onCreated, onUpdate, onDelete }: Props) {
    ...
    const addTag = async () => {
        ...
        const { tag, status } = await createTag({
            tagName: draftName.trim(),
            colorCode: draftColor,
        });

        if (status !== 200 && status !== 201) {
            toast.show('info', 'タグの作成に失敗しました');
            return;
        }

        toast.show('success', `タグ「${tag?.tagName ?? draftName.trim()}」を作成しました`);
        setDraftName('');
        onCreated?.();
    };
    ...
    const startEdit = (t: TagResponse) => {
        ...
        setEditingName(t.tagName);
        setEditingColor(t.colorCode);
    };
            ...
            <div className="bg-background-100 rounded-xl p-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <div className="sm:mt-6">
                        <Button onClick={addTag} disabled={!draftName.trim() || isCreating}>
            ...
            <div className="mt-5">
                ...
                {tags.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                        {tags.map((t) => (
                            <li key={t.tagId} className="flex items-center gap-3 bg-background-100 rounded-xl px-4 py-3">
                                {editingId === t.tagId ? (
                                    ...
                                    <>
                                        <span className="w-4 h-4 rounded-full shrink-0" style={{ backgroundColor: t.colorCode }} />
                                        <span className="font-bold text-sm text-foreground-800 flex-1 truncate">{t.tagName}</span>
                                        {confirmDelete === t.tagId ? (
                                            <div className="flex items-center gap-1.5">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => setConfirmDelete(t.tagId)}
